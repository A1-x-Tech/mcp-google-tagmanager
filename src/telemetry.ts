 ...
 * account/container ids and paths, tool arguments, prompts and the names or
 * values of environment variables are never read, serialized or sent. Sends
 * are capped at 2 s and swallow every error — telemetry must never affect the
 * server's operation. Opt out for every server by this author at once:
 * ASKADS_TELEMETRY=0 (also accepts false/off/no).
 */
...
const APP = "mcp-google-tagmanager";
 ...
 * and still means "config unusable at load" (now: some credential variable set
 * but no workable combination); it is kept unchanged so the historical funnel
 * stays comparable. `unconfigured_start` is its live counterpart — the server
 * now survives missing credentials and completes the handshake, so this is the
